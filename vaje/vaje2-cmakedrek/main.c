...
int main(int argc, char **argv){
	...
	*/
	while (1)
	{
		...
		arr_a[thread_c].ID=thread_c;
		arr_a[thread_c].running=1;
		arr_a[thread_c].clientsock=clientSock;
		pthread_create(&arr_t[thread_c],NULL,user_connect_t,&arr_a[thread_c]);
		close(clientSock);
	}
